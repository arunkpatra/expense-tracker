...
public class JasperReportGenerationService implements ReportGenerationService {
	...
	/**
	 * This is a pretty heavyweight process. So, we will execute it asynchronously
	 */
	@Transactional
	@Async
	...
	private byte[] genSettlementReportInternal(Long sid, RequestContext ctx) {
		...
		try {
			...
			isJrxmlFile = new FileInputStream(context
					.getRealPath(SETTLEMENT_REPORT_FILE_NAME));
			...
			Map filParams = getSettlementReportParamMap(sid);
			// fill now
			fillInputStream = new ByteArrayInputStream(compilerOutputStream
					.toByteArray());
	...
	private byte[] genExpenseReportInternal(Long sid, RequestContext ctx) {
		...
		try {
			// get the input file contents as a stream
			isJrxmlFile = new FileInputStream(context
					.getRealPath(EXPENSE_REPORT_FILE_NAME));
			JasperCompileManager.compileReportToStream(isJrxmlFile,
					compilerOutputStream);
			// compile completed
			// get ready to fill
			...
			Map filParams = getExpenseReportParamMap(sid);
			// fill now
			fillInputStream = new ByteArrayInputStream(compilerOutputStream
					.toByteArray());
		...
		System.out
				.println("*** Expense report generated and saved to DB");
		return pdfBytes;
	}
	@Transactional
